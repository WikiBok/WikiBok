...
class BokHistoryList extends IncludableSpecialPage {
	/**
	 * コンストラクタ
	 */
	public function __construct() {
		global $wgOut;
		parent::__construct('BokHistoryList');
		$wgOut->setPageTitle(wfMsg('bokhistorylist'));
	}
	/**
	 * 出力実行
	 */
	public function execute( $target ) {
		...
		//DB問い合わせ
		$db = new RevisionDB();
		$rows = $db->getHistoryList($from,$to,$user);
		if($rows != FALSE) {
			//データ表示
			$out->addHTML(
				$this->_pager().
			...
			$out->addHTML(
				...
				Xml::openElement('tr').
				Xml::element('th',null,wfMsg('wbs-list-result-rev')).
			...
			foreach($rows as $row) {
				$out->addHTML(
					Xml::openElement('tr').
					Xml::element('td',null,$row['rev']).
					Xml::element('td',null,User::whoIs($row['user_id'])).
					Xml::element('td',null,$row['time']).
					Xml::openElement('td').
					xml::element('a',array('href'=>$url.'/Special:BokEditor?action=history&rev='.$row['rev'],'target'=>'_blank'),'○').
					Xml::closeElement('td').
					Xml::closeElement('tr')
				);
			}
	...
	/**
	 * ページャHTML作成
	 * @param $selected 1ページに表示する件数[選択値]
	 */
